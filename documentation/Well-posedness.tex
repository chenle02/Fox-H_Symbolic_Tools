...
\begin{document}
  \begin{tikzpicture}[scale=1, transform shape, node distance = 4em, auto]
    ...
    \node[poles] (poles) at (0,0) {No overlap between $a_{ik}$ poles and $b_{j\ell}$ poles};
    ...
    \node[below of = D000, xshift = +9em, yshift = -2em] (b) {$b_{j\ell}$ poles};
    \node[below of = D000, xshift = -9em, yshift = -2em] (a) {$a_{ik}$ poles};

    \node[below of = b, yshift = +1em] (bf) {$ \displaystyle H_{p,q}^{m,n}(z) = \sum_{j=1}^{m} \sum_{\ell=0}^{\infty} \Res_{s = b_{j\ell}} \left[\mathcal{H}_{p,q}^{m,n}(s) z^{-s}\right]$};
    \node[below of = a, yshift = +1em] (af) {$ \displaystyle H_{p,q}^{m,n}(z) = \sum_{i=1}^{n} \sum_{k=0}^{\infty}    \Res_{s = a_{ik}}    \left[\mathcal{H}_{p,q}^{m,n}(s) z^{-s}\right]$};
